...
import React from 'react';
import { BrowserRouter as Router, Routes, Route, useLocation } from 'react-router-dom'; // <--- Agregamos useLocation
import { AuthProvider } from './context/authContext';
import Login from './components/home/Login';
import Register from './components/home/Register';
import Dashboard from './components/dashboard/Dashboard';
...
// Wrapper para inyectar la key dinámica basada en la URL completa (?type=...&data=...)
const InteractiveTutorialWrapper = () => {
  const location = useLocation();
  // Al cambiar la key, React DESTRUYE y RECREA el componente de cero (Efecto F5)
  return <InteractiveTutorial key={location.pathname + location.search} />;
};
    ...
    <Router>
      ...
      <NavigationBar /> 
        <Routes>
          ...
          {/* Reemplazamos <InteractiveTutorial /> por <InteractiveTutorialWrapper /> */}
          <Route path="/interactive-tutorial" element={<PrivateRoute><InteractiveTutorialWrapper /></PrivateRoute>} />
