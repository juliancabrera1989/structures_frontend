...
const CommentList = ({ comments = [] }) => {
  ...
  return (
    <ListGroup variant="flush" className="rounded">
      {comments.map((c) => {
        const isCommentAdmin = c.author?.role === 'admin' || c.authorRole === 'admin';
              ...
              <div>
                <div className="d-flex align-items-center gap-2">
                  <strong style={{ color: '#00d8ff' }}>
                    {c.author?.username || c.author?.email || 'Anónimo'}
                  </strong>

                  {/* 👈 Tag de Admin visible en el Comentario */}
                  {isCommentAdmin && (
                    <span 
                      className="badge bg-info text-dark fw-bold px-2 py-1" 
